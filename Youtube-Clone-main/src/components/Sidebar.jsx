...
function Sidebar() {
  ...
  const { selectedCategory, sidebarExtended } = useSelector((state) => state.category);
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <div
        className={`fixed top-14 left-0 h-[calc(100vh-56px)] z-20 transition-all duration-300 ${
          sidebarExtended ? "w-60" : "w-16"
        } ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"} hidden sm:block overflow-y-auto`}
      >
        <div className="flex flex-col p-2">
          {categories.map((category) => (
            ...
            >
              <span className={sidebarExtended ? "mr-4" : ""}>
                {selectedCategory === category.name ? category.active : category.icon}
              </span>
              {sidebarExtended && (
      ...
      <div
        ...
        } ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-900"} sm:hidden`}
      >
        {/* Mobile Header */}
